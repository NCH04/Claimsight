...
from .config import DEFAULT_SEVERITY_CLASSES, IMAGENET_MEAN, IMAGENET_STD
...
class SeverityModel:
    def __init__(self, checkpoint_path: Optional[str] = None, device: str = "cuda" if torch.cuda.is_available() else "cpu"):
        ...
        if checkpoint_path is None or not os.path.exists(checkpoint_path):
            self.model = None
            self.classes = DEFAULT_SEVERITY_CLASSES
            self.tf = None
            self.is_dummy = True
            return
    ...
    def _load_checkpoint(self) -> Tuple[nn.Module, List[str], transforms.Compose]:
        ...
        classes: List[str] = ckpt.get("classes") or ckpt.get("labels") or DEFAULT_SEVERITY_CLASSES
        ...
        if state_dict is None:
            raise ValueError(
                f"Invalid severity checkpoint at {self.checkpoint_path}. "
